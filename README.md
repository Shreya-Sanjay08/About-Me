# Hi, I'm Shreya Sanjay 👋

🎓 Computer Systems Engineering Graduate — 
Middlesex University Dubai (2026)
🏆 Future Hack 2026 Winner | Top 10 Changemakers 
Academy 2026 | URIC 2026 Finalist
🚀 Building at the intersection of IoT, AI and 
Full Stack Development
📍 Dubai, UAE | Available Immediately

---

## 🛠️ Tech Stack

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

**Web & Backend**
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat&logo=firebase&logoColor=black)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)

**AI & Data Science**
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)

**IoT & Hardware**
- Arduino Nano ESP32 | Raspberry Pi | MPU6050
- RFID | PIR & LDR Sensors | LCD/OLED

**Tools & Automation**
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat&logo=n8n&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## 🏆 Featured Projects

### 🥇 BackTrix — AI Smart Posture Wearable
> 1st Place Future Hack 2026 | Top 10 Changemakers 
Academy 2026 (200+ UAE applicants)

B2B IoT wearable ecosystem using ESP32 + MPU6050 
for real-time spinal posture monitoring. Features 
enterprise HR dashboard and gamified reward system 
(SpineCoins).

**Tech:** Arduino Nano ESP32 • MPU6050 • Python 
- Firebase

---

### 🔍 ForensiTwin — AI Forensic Analysis Platform
> Finalist — URIC 2026

Full-stack AI forensic documentation system with 
immersive 360° crime scene navigation, SHA-256 
evidence integrity, Ollama LLM integration and 
n8n automation workflows.

**Tech:** React.js • FastAPI • PostgreSQL • 
SHA-256 • Ollama • n8n

---

### 🤖 MemoryBot — Multi-Provider AI Chatbot
> Live Demo: [memorybot.vercel.app](https://memory-bot-an-ai-chatbot.vercel.app/)

AI chatbot with full conversation memory supporting 
5 providers — Ollama (free local), Groq, OpenRouter, 
OpenAI and Anthropic. Features 6 personality presets, 
pinned memories, chat export and session analytics. 
Built with adapter pattern architecture for clean 
multi-provider support.

**Tech:** HTML5 • CSS3 • Vanilla JavaScript • 
Ollama • Groq • OpenAI API • Anthropic API

---

### 🏫 Smart Classroom — IoT Learning Environment
End-to-end IoT classroom automation with RFID 
attendance for 30+ students, automated door 
security and PIR/LDR sensor control.

**Tech:** ESP32 • RFID • Firebase • PIR & LDR 
Sensors • Web Dashboard

---

### 🛍️ SAMOriginals — E-Commerce Platform
> Live Demo : [SAMOriginals.vercel.app](https://uae-streetwear-store.vercel.app/)
Fully functional multi-page e-commerce SPA for 
UAE streetwear brand with WhatsApp Business API 
checkout and live drop countdown.

**Tech:** HTML5 • CSS3 • Vanilla JS • 
WhatsApp Business API

## 📜 Certifications

- 🏅 Azure AI Fundamentals AI-900 — Microsoft
- 🏅 Azure Fundamentals AZ-900 — Microsoft
- 🏅 AI Fundamentals — IBM SkillsBuild
- 🏅 Introduction to Modern AI — Cisco
- 🏅 Introduction to IoT — Cisco
- 🏅 PMI Project Management Fundamentals — PMI
- 🏅 Python Essentials 1 & 2 — Cisco
- 🏅 Fortinet Cybersecurity Fundamentals

---

## 🏆 Achievements

| Award | Year |
|---|---|
| 🥇 1st Place — Future Hack 2026 | 2026 |
| 🏆 Top 10 — Changemakers Academy UAE | 2026 |
| 🎓 Finalist — URIC 2026 | 2026 |

---

![GitHub Streak](https://streak-stats.demolab.com?user=Shreya-Sanjay08&theme=tokyonight&hide_border=true)

---
## 🤝 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shreyasanjay08s/)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:shreyasanjay811@gmail.com)
[![Credly](https://img.shields.io/badge/Credly-FF6B00?style=flat&logo=credly&logoColor=white)](https://www.credly.com/users/shreya-sanjay.cef25cdc)

---

> *"Build things that matter. Ship things that work."* 🚀
